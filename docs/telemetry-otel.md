# OpenTelemetry Traces (Agent)

The agent emits OTLP traces alongside the hub. Same gate semantics as the hub, same standard `OTEL_*` env vars — see the hub's `docs/telemetry-otel.md` for the full configuration reference.
...
- Structured-log export (separate milestone, mirroring the hub-side `slog` -> OTel logs bridge)
...
Trigger an HTTP check from the hub, then look for spans tagged `service.name=watchdog-agent` in the receiver UI.
